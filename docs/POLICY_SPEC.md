...
| 등급(grade) | 가격 | 권한 |
|---|---|---|
| **무료 체험(free)** | 0원 | 30분 1회 + 통역 + 용어집 |
| **1시간권(1h)** | (확정 요금) | 통역 + 용어집 |
| **5시간권(5h)** | (확정 요금) | 통역 + 용어집 (시간만 차이) |
| **10시간권(10h)** | (확정 요금) | 통역 + 용어집 (시간만 차이) |

> 🔄 **2026-07-04 정책 변경(교수님 확정)**: 용어집(저장·내 용어집·클라우드 동기화·내보내기·불러오기)은
> **시간권 구별 없이 전 등급 영구 개방** — 유저 리텐션 전략. 등급은 통역 시간만 차등.
...
```js
const canUseGlossary = true   // 2026-07-04 정책 변경: 전 등급 영구 개방 (리텐션 전략)
