...
(function () {
  ...
  var DICT = {
    ...
    '자막은 통역 중 30초마다·정지 시 자동 저장(최대 50개). 내보내기는 현재 화면 자막을 txt로, 전체 삭제는 저장된 자막을 모두 지웁니다. 목록에서 항목 위에 마우스를 올리면 첫 5줄이 미리 보이고(제목 줄을 누르면 고정), 전체 내용은 내보내기로 확인합니다.': 'Subtitles are auto-saved every 30s while interpreting and on stop (up to 50). Export saves the current on-screen subtitles as txt; Delete all removes every saved subtitle. Hover over an item to preview its first 5 lines (click the title row to pin); use Export to see the full content.',
